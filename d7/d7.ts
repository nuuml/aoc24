...
const womboCombo = (left: number, right: number): number => parseInt(`${left}${right}`)

const checkValidityA = (goal: number, nums: number[], current: number): number => {
    if (nums.length == 0) {
    ...
    return checkValidityA(goal, nums.slice(1), current + nums[0]) == goal ||
        checkValidityA(goal, nums.slice(1), current * nums[0]) == goal ? goal : 0
}

const checkValidityB = (goal: number, nums: number[], current: number): number => {
    if (nums.length == 0) {
        return current === goal ? current : 0
    }
    if (current > goal) {
    ...
    return checkValidityB(goal, nums.slice(1), current + nums[0]) == goal ||
        ...
        checkValidityB(goal, nums.slice(1), womboCombo(current, nums[0])) == goal ? goal : 0
}
...
data.forEach((row) => {
    ...
    const tools = unjoined[1].split(' ').slice(1).map((num) => parseInt(num))
